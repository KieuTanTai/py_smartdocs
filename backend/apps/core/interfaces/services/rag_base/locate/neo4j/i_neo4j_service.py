...
class INeo4jService(ABC):
    ...
    async def execute_file_to_kg_pipeline(
        ...
        :param params: List of graph RAG parameters
    ...
    @abstractmethod
    def wait_for_index_ready(self, index_name: uuid.UUID, timeout: int = 60, file_caller="") -> bool:
        """
        Waits for the specified vector index to be ready for use in the Neo4j database.
        This method can be used to ensure that the index has been fully created and is available for search operations before proceeding with any queries that rely on the index.
        :param index_name: Name of the index to wait for
        :param timeout: Maximum time to wait for the index to be ready (in seconds)
        :param file_caller: Optional string to identify the caller of this method for logging purposes
        :return: True if the index is ready within the timeout period, False otherwise
        """
        pass
    ...
    def search(self, search_request: INeo4jSearchRequest, llm: LLMInterface, retriever: VectorCypherRetriever, template: RagTemplate, file_caller="") -> str:
        ...
        :param search_request: The search request containing the query and parameters.
    ...
    @abstractmethod
    def create_graph_retriever(self, template: str, embedder: Embedder, index_name: uuid.UUID, file_caller="") -> VectorCypherRetriever:
        """
        Creates a graph retriever based on the provided RAG template, embedder, and index name.
        This method can be used to create a retriever that is configured to retrieve relevant information from the Neo4j database using the specified template and embedding strategy, allowing for flexible retrieval operations based on different templates and embedding configurations.
        :param template: The RAG template to use for formatting the prompt for the LLM based on the retrieved information.
        :param embedder: The embedder to use for creating the retriever.
        :param index_name: The name of the vector index to use for retrieval.
        :param file_caller: Optional string to identify the caller of this method for logging purposes.
        :return: A VectorCypherRetriever instance configured with the provided parameters.
        """
        pass
