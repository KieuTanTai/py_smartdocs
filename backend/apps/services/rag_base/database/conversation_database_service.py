...
class ConversationDatabaseService(IConversationDatabase):
    ...
    def create_conversation(
        ...
        **extra_fields: Any,
        ...
        return self.create(
            ...
            conversation_faiss_index=document,
            **extra_fields,
        )
    ...
    def get_by_name(self, conversation_name: str) -> QuerySet[ConversationModel]:
        return ConversationModel.objects.filter(conversation_name=conversation_name)

    def get_by_document(self, document: DocumentModel) -> QuerySet[ConversationModel]:
        return ConversationModel.objects.filter(conversation_faiss_index=document)

    def list(self, **filters: Any) -> QuerySet[ConversationModel]:
        queryset = ConversationModel.objects.all()
        if filters:
            queryset = queryset.filter(**filters)
        return queryset.order_by("-conversation_created_at")
