...
class MessageListViewByConversation(APIView):
    def get(self, request):
        ...
        try:
            ...
            return message_app.get_conversation_messages(str(request.data.get("conversation_id")),50,0,Path(__file__).name)
        except ValueError as e:
    ...
    def post(self, request):
        ...
        try:
            ...
            return message_app.send_message(str(request.data.get("conversation_id")), send_msg_req.user_input, send_msg_req.provider_name, send_msg_req.model_name, send_msg_req.pipeline_type, Path(__file__).name)
