...
- `frontend/components/account/login.py` for handle form login popup, send request login to backend
- `frontend/components/account/signup.py`for handle form signup popup, send request signup to backend
...
- `frontend/components/header.py` for handle header of website, flex layout, have web icon, web name on left side, button setting for calling `frontend/components/settings/system_settings.py` when clicked, circle button with account avatar (default using on `frontend/assets/`), if not login, this button will show login/signup option when clicked, calling (`frontend/components/account/login.py` / `frontend/components/signup.py` form when clicked)  
...
- System settings button, account button is highest z-index for not overlap with right side bar. Account button dropdown just show login/signup button, when click it will open form overlay, when has logined will show logout button
...
- Click outside dropdown form will close form (include account dropdown list buttons)
